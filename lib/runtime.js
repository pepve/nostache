...
function render (directory, filename, view, cb) {
	...
	var state = { directory: directory + '/', buffers: [], depth: 0, cache: {} };
...
function runTemplate (state, context, filename, cb) {
	...
	// We cache all templates that are used within one render call unconditionally
	if (state.cache.hasOwnProperty(absFilename)) {
		runList(state, absFilename, context, state.cache[absFilename].ast.list, cb);
		...
		fs.stat(absFilename, function (err, stats) {
			...
			// The global cache is invalidated by any file modification
			if (cache.hasOwnProperty(absFilename) && cache[absFilename].mtime >= stats.mtime) {
				state.cache[absFilename] = cache[absFilename];
				...
				fs.readFile(absFilename, { encoding: 'utf8' }, function (err, text) {
					...
					cache[absFilename] = state.cache[absFilename] = { mtime: stats.mtime, ast: ast };
...
function resolve (context, key, cb) {
	...
	(function resolveLoop (i, object) {
		materialize(object, function (err, object) {
			...
			} else if (object instanceof LazyObject) {
				// There is a current object and it is lazy, so we interlocute it
				if (object.values.hasOwnProperty(nth)) {
					resolveLoop(i + 1, object.values[nth]);

				} else {
					process.nextTick(object.getter.bind(null, nth, function (err, res) {
						if (err) {
							return cb(err);
						}

						object.values[nth] = res;
						resolveLoop(i + 1, res);
					}));
				}

			} else if (object.hasOwnProperty(nth)) {
				// There is a current object and it has a property with the current key part
				resolveLoop(i + 1, object[nth]);

			} else {
				// It doesn't have a property with the current key part
				cb();
			}
		});
	}(1, object));
